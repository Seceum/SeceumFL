...
在开始阶段，我们强烈建议用两台物理机以及docker来部署**SeceumFL**。    
...
从github上拉取项目代码到服务器上如：/home/alice/。  
从腾讯云上拉取docker镜像.   
```shell
cd /home/alice/
git clone https://github.com/Seceum/SeceumFL.git
docker pull seceum-fl:3.2
docker pull seceum-fl-web:3.2
```
...
```
如果脚本运行没有问题，可以跳过其他配置的步骤，也可以再check一下以下的配置内容。
...
## 4.    启动服务
这里
要启动10个服务，可以用**docker-compose**，但建议一个个服务启动更为妥当。
...
### 4.1  周边服务的启动
这里需要启动**mysql、zookeeper、redis（非必须）**
...
### 4.2  Eggroll的启动
这里需要启动三个服务：**rollsite、clustermanager、nodemanager**
...
### 4.3  WEB服务的启动
这里需要启动两个服务，一个是**seceum-fl-web**，另外一个是FATE-Board。
```shell
...
### 4.4 FateFlow后台服务的启动
这里需要启动两个服务：**fate_flow_server、serving（在线预测）**。
```shell
...
## 5.    服务运行状态的确认
首先，需要确认一下，是否以下这10个服务都已经在线，然后再确认服务的内部状态是否健康。
...
```
进入后，执行以下脚本(gid为当前机器的PartyID， hid为合作方PartyID)，如最后能得到**success**，表明系统已经通过冒烟测试。
