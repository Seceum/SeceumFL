...
start() {
    ...
    if [[ ${pid} == "" ]]; then
        ...
        if [[ $1x == "front"x ]];then
          ...
          exec python3 ${FATE_FLOW_BASE}/python/fate_flow/fate_flow_server.py >> "${log_dir}/console.log" 2>>"${log_dir}/error.log"
          exec python3 ${FATE_FLOW_BASE}/python/chain/chain_start.py >> "${log_dir}/fate_flow/chain.log" 2>>"${log_dir}/fate_flow/chain.log" &
          unset FATE_PROJECT_BASE
        else
          export FATE_PROJECT_BASE=${PROJECT_BASE}
          nohup python3 ${FATE_FLOW_BASE}/python/fate_flow/fate_flow_server.py >> "${log_dir}/console.log" 2>>"${log_dir}/error.log" &
          nohup python3 ${FATE_FLOW_BASE}/python/chain/chain_start.py >> "${log_dir}/fate_flow/chain.log" 2>>"${log_dir}/fate_flow/chain.log" &
          unset FATE_PROJECT_BASE
        fi
        for((i=1;i<=100;i++));
        do
...
stop() {
    ps -aux |grep chain_start.py |grep python|grep -v grep|awk '{print $2}' |xargs kill -9
    getpid
    if [[ -n ${pid} ]]; then
        ...
        if [[ $? -eq 0 ]]; then
            ...
            echo "service stop 9360 success"
        else
            echo "kill error"
        fi
    else
