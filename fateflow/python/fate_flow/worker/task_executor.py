...
from federatedml.statistic.intersect.bark_oprf_intersect.bark import BaRKGuest, BaRKHost
...
class TaskExecutor(BaseTaskWorker):
    def _run_(self):
        ...
        try:
            ...
            if component.get_module() == "Intersection":
                _, _, user_specified_parameters = \
                ...
                if user_specified_parameters.get("ComponentParam",{}).get("intersect_method","") == "bark-oprf":
                    LOGGER.info(">>>>Before Bark: {}".format(current_timestamp() - start_time))
                    return self.bark(args, job_parameters, dsl_parser, component.get_input(),
                                     component.get_output(), job_args_on_party, user_name)
            ...
            LOGGER.error(f"####seceum_error_start####")
            LOGGER.error(f"{repr(e)}")
            LOGGER.error(f"####seceum_error_end####")
            traceback.print_exc()
    ...
    def bark(self, args, job_parameters, dsl_parser, task_input_dsl, task_output_dsl, job_args_on_party, user_name):
        ...
        kwargs = {
            ...
            'model_id': job_parameters.model_id,
            'model_version': job_parameters.model_version,
            'component_module_name': "Intersection",
            'job_parameters': job_parameters,
        }
        tracker = Tracker(**kwargs)
        tracker_client = TrackerClient(**kwargs)
        ...
        LOGGER.info(">>>>Before Bark: {}, init_config".format(current_timestamp() - start_time))
        RuntimeConfig.init_config(COMPUTING_ENGINE=job_parameters.computing_engine,
                                  FEDERATION_ENGINE=job_parameters.federation_engine,
                                  FEDERATED_MODE=job_parameters.federated_mode)

        if RuntimeConfig.COMPUTING_ENGINE == ComputingEngine.EGGROLL:
            session_options = job_parameters.eggroll_run.copy()
            session_options["python.path"] = os.getenv("PYTHONPATH")
            session_options["python.venv"] = os.getenv("VIRTUAL_ENV")
        else:
            session_options = {}
        ...
        LOGGER.info(f"component parameters on party:\n{json_dumps(component_parameters_on_party, indent=4)}")
        LOGGER.info(f"task input dsl {task_input_dsl}")
        LOGGER.info(">>>>Before Bark: {}, get_task_run_args".format(current_timestamp() - start_time))
        ...
        LOGGER.info(">>>>Before Bark: {}, what the fuck".format(current_timestamp() - start_time))
        bark = BaRKGuest(args.job_id, user_specified_parameters["role"]["host"]) if args.role == "guest" else BaRKHost(args.job_id)
        bark.load_params(IntersectParam(**user_specified_parameters.get("ComponentParam",{})))
        out_datainstances = bark.run_intersect(task_run_args["data"]["reader"]["data"][0])
        sess.wait_remote_all_done()
        tracker_client.log_component_summary(summary_data=bark.summary())
        bark.callback(tracker_client)
        LOGGER.info(">>>>Bark Done: {}".format(current_timestamp() - start_time))

        output_table_list = []
        data_name = task_output_dsl.get('data')[0] if task_output_dsl.get('data') else '{}'.format(0)
        persistent_table_namespace, persistent_table_name = tracker.save_output_data(
            computing_table=out_datainstances,
            output_storage_engine=job_parameters.storage_engine,
            token={"username": user_name})

        LOGGER.info(">>>>After Bark save_output_data: {}".format(current_timestamp() - start_time))
        if persistent_table_namespace and persistent_table_name:
            tracker.log_output_data_info(data_name=data_name,
                                         table_namespace=persistent_table_namespace,
                                         table_name=persistent_table_name)
        ...
        LOGGER.info(">>>>After Bark log_output_data_info: {}".format(current_timestamp() - start_time))
        self.log_output_data_table_tracker(args.job_id, input_table_list, output_table_list)
        ...
        self.report_task_info_to_driver()
        sess.destroy_all_sessions()
        LOGGER.info(">>>>After Bark log_output_data_table_tracker: {}".format(current_timestamp() - start_time))
        return self.report_info
