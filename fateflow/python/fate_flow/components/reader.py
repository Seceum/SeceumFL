...
class Reader(ComponentBase):
    ...
    def _run(self, cpn_input: ComponentInputProtocol):
        ...
        if re.search(r"[0-9]{10,}_outs", input_table_namespace):
            return self.__run(cpn_input, input_table_namespace, input_table_name)
    ...
    def __run(self, cpn_input, input_table_namespace, input_table_name):
        output_storage_address = self.job_parameters.engines_address[EngineType.STORAGE]
        # only support one input table
        table_key = [key for key in self.parameters.keys()][0]
        ...
        input_table_meta = StorageTableMeta(name=input_table_name, namespace=input_table_namespace)
        sess = Session.get_global()
        input_table = sess.get_table(
            name=input_table_meta.get_name(), namespace=input_table_meta.get_namespace()
        )
        ...
        self.tracker.log_output_data_info(
            ...
            table_namespace=input_table_meta.get_namespace(),
            table_name=input_table_meta.get_name(),
        )
        DataTableTracker.create_table_tracker(
            input_table_meta.get_name(),
            input_table_meta.get_namespace(),
            entity_info={
                "have_parent": True,
                "parent_table_namespace": input_table_namespace,
                "parent_table_name": input_table_name,
        ...
        table_info, anonymous_info, attribute_info = self.data_info_display(input_table_meta)
        data_info = {
            "table_name": input_table_name,
            "namespace": input_table_namespace,
            "table_info": table_info,
            "anonymous_info": anonymous_info,
            "attribute_info": attribute_info,
            "partitions": input_table_meta.get_partitions(),
            "storage_engine": input_table_meta.get_engine(),
        }
        if input_table_meta.get_engine() in [StorageEngine.PATH]:
            data_info["file_count"] = input_table_meta.get_count()
            data_info["file_path"] = input_table_meta.get_address().path
        else:
            data_info["count"] = input_table_meta.get_count()
