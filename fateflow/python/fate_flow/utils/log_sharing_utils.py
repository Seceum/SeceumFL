...
class LogCollector():
    ...
    def cat_log(self, begin=300, end=None):
        line_list = []
...
class ErrorLogCollector(LogCollector):
    ...
    def __init__(self, log_type, job_id, party_id="", role="", component_name="", **kwargs):
        self.error_start = "####seceum_error_start####"
        self.error_end = "####seceum_error_end####"
        super(ErrorLogCollector, self).__init__(log_type, job_id, party_id, role, component_name=component_name, **kwargs)
    ...
    def parse_error_info(self, line_list):
        error_info = []
        find_error = False
        for line in line_list:
            if find_error is True:
                error_line = line['content'].split(":")[-1]
                error_line = error_line.strip()
                error_info.append(error_line)
                find_error = False
                break
            if line['content'].find(self.error_start) != -1:
                find_error = True
