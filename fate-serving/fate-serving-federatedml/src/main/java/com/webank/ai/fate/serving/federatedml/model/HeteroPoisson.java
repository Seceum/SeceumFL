...
import com.webank.ai.fate.core.mlmodel.buffer.PoissonModelParamProto.PoissonModelParam;
import com.webank.ai.fate.core.mlmodel.buffer.PoissonModelMetaProto.PoissonModelMeta;
...
public abstract class HeteroPoisson extends BaseComponent {
    private static final Logger logger = LoggerFactory.getLogger(HeteroPoisson.class);
    private Map<String, Double> weight;
    private Double intercept;
    protected String exposure_colname;
    PoissonModelParam poissonModelParam;
    PoissonModelMeta poissonModelMeta;

    @Override
    public int initModel(byte[] protoMeta, byte[] protoParam) {
        logger.info("start init HeteroPoisson class");
        try {
            poissonModelParam = this.parseModel(PoissonModelParam.parser(), protoParam);
            poissonModelMeta = this.parseModel(poissonModelMeta.parser(), protoMeta);
            this.weight = poissonModelParam.getWeightMap();
            this.intercept = poissonModelParam.getIntercept();
            this.exposure_colname = poissonModelMeta.getExposureColname();
        } catch (Exception ex) {
            ex.printStackTrace();
            return ILLEGALDATA;
        }
        logger.info("Finish init HeteroPoisson class, model weight is {}", this.weight);
        return OK;
    }
