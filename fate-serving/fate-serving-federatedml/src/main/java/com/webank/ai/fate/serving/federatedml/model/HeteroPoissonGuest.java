...
public class HeteroPoissonGuest extends HeteroPoisson implements MergeInferenceAware, Returnable {

    private static final Logger logger = LoggerFactory.getLogger(HeteroPoissonGuest.class);
    private double safeLog(double x) {
        if(x <= 0)return log(1e-7);
        return log(x);
    }
    @Override
    public Map<String, Object> localInference(Context context, List<Map<String, Object>> input
        ...
        if (this.exposure_colname != null && input.get(0).containsKey(this.exposure_colname)){
            Double m = new Double(input.get(0).get(this.exposure_colname).toString());
            score += safeLog(m);
        }
        score = exp(score);
            ...
            if (CollectionUtils.isNotEmpty(guestData)) {
                ...
                hostData.forEach((k, v) -> {
                    Map<String, Object> onePartyData = (Map<String, Object>) v;
                        ...
                        score.updateAndGet(v1 -> new Double((double) (v1 * exp(remoteScore))));
                    });
                double prob = score.get();
