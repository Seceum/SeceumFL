...
def main(config="../../config.yaml", namespace=""):
    ...
    guest_dense_data = {"name": "breast_homo_guest", "namespace": f"experiment{namespace}"}
    host_dense_data = {"name":"breast_homo_host","namespace":f"experiment{namespace}"}
    ...
    pipeline_upload.add_upload_data(file=os.path.join(data_base, "examples/data/breast_homo_guest.csv"),
                                    table_name=guest_dense_data["name"],             # table name
                                    namespace=guest_dense_data["namespace"],         # namespace
                                    head=1, partition=partition,               # data info
                                    id_delimiter=",")
    pipeline_upload.add_upload_data(file=os.path.join(data_base, "examples/data/breast_homo_host.csv"),
                                    table_name=host_dense_data["name"],  # table name
                                    namespace=host_dense_data["namespace"],  # namespace
                                    head=1, partition=partition,  # data info
                                    id_delimiter=",")
