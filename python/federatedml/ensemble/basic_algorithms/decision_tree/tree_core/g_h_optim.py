...
class GHPacker(object):
    ...
    def __init__(self, sample_num: int, en_calculator: Union[PaillierEncrypt, IpclPaillierEncrypt,EncryptModeCalculator],
                       precision=fix_point_precision, max_sample_weight=1.0, task_type=consts.CLASSIFICATION,
                       h_max=None, g_min=None, g_max=None, sync_para=True):
    ...
    def decompress_and_unpack(self, split_info_package_list):
        ...
        for split_info in rs:
            g, h = g_h_recover_post_func(self.packer.unpack_an_int(split_info.sum_grad, self.packer.bit_assignment[0]),
