...
from federatedml.ensemble.boosting.boosting_core.hetero_boosting import HeteroBoostingGuest
from federatedml.ensemble.boosting.boosting_core.hetero_boosting import HeteroBoostingHost
...
from federatedml.transfer_variable.transfer_class.hetero_xgboost_predict_transfer_variable import \
    HeteroXGBoostTransferVariable
...
from federatedml.util import compute
from federatedml.ensemble.boosting.hetero.hetero_xgboost_host import *


class HeteroXGBoostGuest(HeteroBoostingGuest):

    def __init__(self):
        super(HeteroXGBoostGuest, self).__init__()
        ...
        self.model_param = HeteroXGBoostParam()
        self.complete_secure = False
        self.data_alignment_map = {}
        self.predict_transfer_inst = HeteroXGBoostTransferVariable()
        self.model_name = 'HeteroXGBoost'
    ...
    def _init_model(self, param: HeteroXGBoostParam):

        super(HeteroXGBoostGuest, self)._init_model(param)
    ...
    def process_sample_weights(self, grad_and_hess, data_with_sample_weight=None):
        ...
        if data_with_sample_weight is not None:
            if with_weight(data_with_sample_weight):
                ...
                grad_and_hess = compute.fun4(grad_and_hess, data_with_sample_weight)
    ...
    def compute_grad_and_hess(self, y_hat, y, data_with_sample_weight=None):
        ...
        if self.task_type == consts.CLASSIFICATION:
            grad_and_hess = compute.fun5(y, y_hat, loss_method)
        else:
            grad_and_hess = compute.fun6(y, y_hat, loss_method)
    ...
    def get_grad_and_hess(g_h, dim=0):
        ...
        grad_and_hess_subtree = compute.fun7(g_h,dim)
        ...
        for id_ in feature_importances:
            ...
            if type(id_) == tuple:
                if consts.GUEST in id_[0]:
                    ...
                    new_fi[Anonymous.generate_anonymous_column(role=role, suf=id_[1], party_id=party_id)] = feature_importances[
                        id_].importance
    ...
    def traverse_trees(node_pos, sample, trees: List[HeteroDecisionTreeGuest]):
        ...
        for t_idx, tree in enumerate(trees):
            ...
            rs, reach_leaf = HeteroXGBoostGuest.traverse_a_tree(tree, sample, cur_node_idx)
    ...
    def get_predict_scores(leaf_pos, learning_rate, init_score, trees: List[HeteroDecisionTreeGuest]
        ...
        predict_func = functools.partial(HeteroXGBoostGuest.add_y_hat,
        ...
        if predict_cache:
            ...
            predict_result = compute.fun8(predict_result, predict_cache)
    ...
    def boosting_fast_predict(self, data_inst, trees: List[HeteroDecisionTreeGuest], predict_cache=None,
        ...
        final_leaf_pos = data_inst.mapValues(
            lambda x: np.zeros(tree_num, dtype=np.int64) + np.nan)  # record final leaf pos

        # final_leaf_pos = compute.fun9(data_inst, tree_num)
        ...
        while True:
            ...
            reach_leaf_samples = compute.fun10(node_pos_tb)
    ...
    def get_model_meta(self):
        ...
        # todo
        meta_name = "HeteroXGBoostGuestMeta"
    ...
    def get_model_param(self):
        ...
        # add by tjx 2023314
        # model_param.model_name = consts.HETERO_XGBOOST
        ...
        feature_name_fid_mapping = self.host_feature_id_mapping
        for (sitename, fid), importance in feature_importances:
            if consts.GUEST in sitename:
                ...
                full_name = Anonymous.generate_anonymous_column(suf=fid, party_id=party_id, role=role_name)

                # update 202362
                fullname = None
                for feature_name in feature_name_fid_mapping:
                    if feature_name.get(str(party_id)):
                        temp = feature_name.get(str(party_id))
                        fullname = '_'.join([temp.get(int(full_name.split('_')[-1])),party_id])
                        break
        ...
        # todo
        param_name = "HeteroXGBoostGuestParam"
        # update by tjx 2023314
        # param_name = "HeteroXGBoostGuestModelParam"
