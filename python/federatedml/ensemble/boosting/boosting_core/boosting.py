    ...
    def data_format_transform(row):
        ...
        if type(row.features).__name__ != consts.SPARSE_VECTOR:
            ...
            for i in range(feature_shape):
                ...
                flag = None
                try:
                    flag = np.isnan(row.features[i])
                except Exception as ex:
                    flag = np.isnan(float(row.features[i]))
                flag1 = None
                if not flag:
                    try:
                        flag1 = np.abs(row.features[i]) < consts.FLOAT_ZERO
                    except Exception as ex:
                        flag1 = np.abs(float(row.features[i])) < consts.FLOAT_ZERO
                if flag:
                    indices.append(i)
                    ...
                    try:
                        ...
                        data.append(float(row.features[i]))
