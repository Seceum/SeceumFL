    ...
    def read_data(self, input_data, mode="fit"):
        ...
        if not schema.get("meta"):
            ...
            if self.exclusive_data_type:
                for idx, col_name in enumerate(schema["header"]):
                    if col_name in self.exclusive_data_type:
                        self.exclusive_data_type_fid_map[idx] = self.exclusive_data_type[col_name]
        else:
    ...
    def to_instance(self, features, label=None):
        ...
        if label is not None:
            if self.label_type == 'int':
                # update by tjx 2023510
                label = int(eval(label)) if label and label!='' else label
    ...
    def gen_output_format(features, data_type='float', exclusive_data_type_fid_map=None,
        ...
        if output_format == "dense":
            ...
            if exclusive_data_type_fid_map:
                ...
                # LOGGER.info("format features type---{}".format(type(format_features)))
                # LOGGER.info("data type content---{}".format(data_type))
                # LOGGER.info("format feature content---{}".format(format_features))
                # for con in format_features:
                #     LOGGER.info("con content---{},type----{}".format(con,type(con)))
                # update by tjx 202347 样本上传的时候，数据中有缺失值
                try:
                    return np.asarray(format_features, dtype=data_type)
                except Exception as ex:
                    return np.asarray(format_features,dtype="str")
