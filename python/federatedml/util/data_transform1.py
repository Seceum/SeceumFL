...
class DenseFeatureTransformer(object):
    def __init__(self, data_transform_param):
        ...
        self.feature = data_transform_param.feature
    ...
    def generate_header(self, input_data, mode="fit"):
        header = None
        sid_name = None
        if "header" in input_data.schema.keys():
            header = input_data.schema["header"]
            if isinstance(header,str):
                header = header.lower()
            elif isinstance(header,list):
                header = ','.join(input_data.schema["header"])
                if "label_name" in input_data.schema.keys():
                    label_name = input_data.schema["label_name"]
                    header = ",".join([header,label_name])

        if "sid" in input_data.schema.keys():
            sid_name = input_data.schema["sid"].lower()
        elif "sid_name" in input_data.schema.keys():
            sid_name = input_data.schema["sid_name"].lower()
    ...
    def extract_feature_value(self, value):
        ...
        if isinstance(value,Instance):
                value1 = ','.join([str(v) if v else "0" for v in value.features])
                if value.label:
                    value = value1+","+str(value.label)
                else:
                    value = value1
    ...
    def read_data(self, input_data, mode="fit"):
        ...
        if self.label_idx is not None:
            # if not isinstance(input_data,Instance):
            #     data_shape = data_overview.get_data_shape(input_data)
            # else:
            #     data_shape = len(input_data.features)
            # LOGGER.info("data shape----{}".format(data_shape))
            # if not data_shape or self.label_idx >= data_shape:
            #     raise ValueError("input data's value is empty or it does not contain a label")
            # if not isinstance(input_data,Table):
            #     input_data_labels = input_data.mapValues(lambda value: value.split(self.delimitor, -1)[self.label_idx])
            # else:
                # if isinstance(value, Instance):
                    # value = ','.join([str(v) if v else None for v in value.features])

            # input_data_labels = input_data.mapValues(lambda value: (','.join([str(v) if v else None for v in value.features])+","+str(value.label)).split(self.delimitor,-1)[self.label_idx]
            #                                          if not isinstance(value, str) else value.split(self.delimitor, -1)[self.label_idx] )
            input_data_labels = input_data.mapValues(lambda value: (
                        ','.join([str(v) if v else "0" for v in value.features]) + "," + str(value.label)).split(
                self.delimitor, -1)[self.label_idx])
            # input_data_labels = input_data.mapValues(lambda  value: value.label)
        ...
        if mode == "fit":
            data_instance = self.fit(input_data, input_data_features, input_data_labels, input_data_match_id,self.feature)
        else:
    ...
    def fit(self, input_data, input_data_features, input_data_labels, input_data_match_id,outlier_feature):
        # outlier_feature which column regard as outlier column add by tjx 202282
        schema = self.get_schema()
        set_schema(input_data_features, schema)

        input_data_features = self.fill_missing_value(input_data_features, "fit")
        input_data_features = self.replace_outlier_value(input_data_features, "fit",outlier_feature)
    ...
    def replace_outlier_value(self, input_data_features, mode="fit",outlier_feature=None):
        if self.outlier_replace:
            from federatedml.feature.imputer1 import Imputer1
            imputer_processor = Imputer1(self.outlier_impute)
            if mode == "fit":
                input_data_features, self.outlier_replace_value = \
                    imputer_processor.fit(input_data_features,
                                          ...
                                          replace_value=self.outlier_replace_value,outlier_feature=outlier_feature)
...
class DataTransform1(ModelBase):
    def __init__(self):
        super(DataTransform1, self).__init__()
        self.transformer = None
        from federatedml.param.data_transform1_param import DataTransform1Param
        self.model_param = DataTransform1Param()
