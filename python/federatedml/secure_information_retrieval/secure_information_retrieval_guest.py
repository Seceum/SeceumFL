...
class SecureInformationRetrievalGuest(BaseSecureInformationRetrieval):
    ...
    def fit(self, data_inst):
        ...
        :return:
        """
        # add by tjx 2022616 convert target ids to data instance
        target_ids = self.model_param.target_ids
        data_in = [(id, Instance(features=id)) for id in target_ids]
        data_inst = session.parallelize(data_in, include_key=True, partition=4)
    ...
    def take_exact_sample(data_inst, exact_num):
        ...
        sample_inst = pd.DataFrame(data_inst.collect())
        sample_inst = sample_inst.sample(exact_num)
        sample_inst.columns = ['x_1','x_2']
        data_in = [(sample_inst.loc[index, 'x_1'], sample_inst.loc[index, 'x_2']) for index in sample_inst.index]
        sample_inst = session.parallelize(data_in, include_key=True, partition=4)
        return sample_inst
