...
class DataTransform1Param(BaseParam):
    """
    ...
    feature:None or list
        element of list can be any type,which column regard as outlier column
    ...
    def __init__(self, input_format="dense", delimitor=',', data_type='float64',
                 ...
                 missing_impute=None, feature=None,outlier_replace=False, outlier_replace_method=None,
    ...
    def check(self):
        ...
        if type(self.missing_fill).__name__ != 'bool':
            raise ValueError("data_transform1 param's missing_fill {} not supported".format(self.missing_fill))
        ...
        if type(self.with_label).__name__ != 'bool':
            raise ValueError("data_transform1 param's with_label {} not supported".format(self.with_label))

        if self.with_label:
            if not isinstance(self.label_name, str):
                raise ValueError("data transform1 param's label_name {} should be str".format(self.label_name))
