...
class HeteroStepwise(object):
    ...
    def check_stop(self, new_host_mask, new_guest_mask, host_mask, guest_mask):
        ...
        if np.array_equal(new_host_mask, host_mask) and np.array_equal(new_guest_mask, guest_mask):
            LOGGER.info("masks not changed, check_stop returns True")
            return True
        # if full model is the best
        if sum(new_host_mask < 1) == 0 and sum(new_guest_mask < 1) == 0 and self.n_step > 0:
            LOGGER.info("masks are full model, check_stop returns True")
            return True
        ...
        if new_total_nv == self.nvmin and total_nv >= self.nvmin:
            LOGGER.info("variable count min reached, check_stop returns True")
            return True
        # if new best reach variable count upper limit
        if self.nvmax is not None:
            if new_total_nv == self.nvmax and total_nv <= self.nvmax:
                LOGGER.info("variable count max reached, check_stop returns True")
                return True
        # if reach max step
        if self.n_step >= self.max_step:
            LOGGER.info("max step reached, check_stop returns True")
            return True
        return False
    ...
    def client_sync_data_info(self, data):
        ...
        # update by tjx 2023525 删除host方特征匿名化
        anonymous_header = data.schema.get('header')
        self.n_count = n
        ...
        while self.n_step <= self.max_step:
            ...
            if self.backward:
                ...
                for curr_host_mask, curr_guest_mask in backward_gen:
                    ...
                    LOGGER.info("step {}, curr_host_mask {}, curr_guest_mask {}".format(self.n_step, curr_host_mask, curr_guest_mask))
            ...
            LOGGER.info("step {}, best_host_mask {}, best_guest_mask {}".format(self.n_step, host_mask, guest_mask))
