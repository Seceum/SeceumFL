...
class HeteroLinRGuest(HeteroLinRBase):
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        data_instances,validate_data = self.prepare_fit(data_instances, validate_data)
    ...
    def predict(self, data_instances):
        ...
        data_instances = self._abnormal_detection(data_instances)
        ...
        for host_pred in host_preds:
            ...
            # if pred is None or pred.count()==0:
            pred = pred.map(lambda k, v: (str(k), v)).join(
                    host_pred.map(lambda k, v: (str(k), v)), lambda g, h: g + h)
