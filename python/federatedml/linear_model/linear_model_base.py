...
class BaseLinearModel(ModelBase):
    ...
    def _abnormal_detection(self, data_instances):
        ...
        def trans(dt):
            k,v = dt.first()
            if type(k) == type(123):
                return dt.map(lambda k,v: (str(k), v))
            return dt

        data_instances1 = data_instances

        if data_instances1:
            data_instances1 = trans(data_instances)
            data_instances1.schema = data_instances.schema

        return data_instances1
        ...
        def _check_overflow(data_iter):
            for _, instant in data_iter:
                ...
                if isinstance(features, SparseVector):
                    ...
                    try:
                        if np.max(np.abs([float(f) for f in features])) > consts.OVERFLOW_THRESHOLD:
                            return True
    ...
    def prepare_fit(self, data_instances, validate_data):
        ...
        if data_instances:
            data_instances = trans(data_instances)

        if validate_data:
            validate_data = trans(validate_data)

        return data_instances,validate_data
