...
class SummaryStatistics(object):
    ...
    def is_nan(self, value):
        ...
        if value in self.abnormal_list or (isinstance(value, float) and np.isnan(value)) or value == NoneType():
            return True
        return False
...
class MissingStatistic(object):
    ...
    def __init__(self, abnormal_lst, missing_val=None):
        ...
        self.abnormal_lst = abnormal_lst
        self.tag_id_mapping, self.id_tag_mapping = {}, {}
        self.dense_missing_val = missing_val

    @staticmethod
    def is_nan(abnormal_lst, value):

        if (value in abnormal_lst) or (value == NoneType()) or (isinstance(value, float) and np.isnan(value)):
            return True
        return False
    ...
    @staticmethod
    def count_feature_ratio(tb, tag_id_mapping, dense_input, abnormal_lst, missing_val=None):
        func = functools.partial(MissingStatistic.map_partitions_count, tag_id_mapping=tag_id_mapping,
                                 abnormal_lst=abnormal_lst,
    ...
    @staticmethod
    def map_partitions_count(iterable, tag_id_mapping, abnormal_lst, dense_input=True, missing_val=None):
        ...
        for k, v in iterable:
            ...
            if dense_input:
                ...
                if missing_val is None:
                    # arr may take NoneType so cannot use np.isnan to judge whether get nan
                    # idx_arr = np.argwhere(~np.isnan(arr)).flatten()
                    for id, ar in enumerate(arr):
                        if MissingStatistic.is_nan(abnormal_lst, ar) is True:
                            continue
                        idx_arr.append(id)
                else:
...
class MultivariateStatisticalSummary(object):
    ...
    def missing_ratio(self):
        self.missing_static_obj = MissingStatistic(self.abnormal_list)
