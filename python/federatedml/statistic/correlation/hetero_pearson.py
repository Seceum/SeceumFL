...
class HeteroPearson(ModelBase):
    ...
    def fit(self, data_instance):
        ...
        # update by tujx 2023518
        # column_anonymous_names = get_anonymous_header(data_instance)
        column_anonymous_names = column_names
        ...
        parties = [
            ...
            Party("host", self.component_properties.host_party_idlist[0] if self.is_guest else self.component_properties.local_partyid),
        ]
        ...
        # update by tjx 2023412
        remainds_indexes1 = select_column_indexes(data_instance,self.model_param.column_indexes,self.model_param.column_names)
        if not self.is_guest:
            fixed_local_corr = fix_local_corr(local_corr, remainds_indexes1, max(remainds_indexes1)+1)
        else:
            fixed_local_corr = fix_local_corr(local_corr, remainds_indexes1, len(column_names))
        ...
        if self.model_param.calc_local_vif:
            ...
            fixed_local_vif = fix_vif(local_vif, remainds_indexes1, max(remainds_indexes1)+1)
        ...
        # cross parties
            ...
            remote_anonymous_names, remote_remainds_indexes = self.sync_anonymous_names(
                column_anonymous_names, remainds_indexes1
            )
            if self.is_guest:
                names = [column_names, remote_anonymous_names]
                remainds_indexes_tuple = (remainds_indexes1, remote_remainds_indexes)

            else:
                names = [remote_anonymous_names, column_names]
                remainds_indexes_tuple = (remote_remainds_indexes, remainds_indexes1)
            ...
            with SPDZ(
                ...
                if self.is_guest:
                    ...
                    corr = corr.T
                    fixed_corr = fix_corr(
                        corr, m2,m1, remainds_indexes_tuple[1], remainds_indexes_tuple[0]
                    )
...
def standardize(data):
    ...
    def toFloat(x):
        try:
            ...
            raise ValueError(f">>>>: 无法将{x}转换为Float类型！")

    def _square(arr):

        if isinstance(arr, (list,tuple,np.ndarray)):
            arr = [toFloat(a) for a in arr]
            return np.array(arr), np.array([a ** 2 for a in arr])

        arr = toFloat(arr)
        return (arr, arr ** 2)

    # update by tjx 2023413
    sum_x, sum_square_x = data.mapValues(lambda x: _square(x)).reduce(
    ...
    def _std(arr, idx = -1):
        ...
        arr = np.array([toFloat(a) for a in arr])
        if idx == -1: return (arr - mu)/sigma
        return (arr[idx] - mu[idx])/sigma[idx]
    ...
    if len(remiands_indexes) < size:
        ...
        return (
                n,
                data.mapValues(lambda x: _std(x, remiands_indexes)),
                remiands_indexes,
                size,
            )
    # update by tjx 2023413
    return n, data.mapValues(lambda x: _std(x)), remiands_indexes, size

# add by tjx 2023412
def select_column_indexes(data_instance,hit_column_indexes,hit_column_names):
    ...
    if hit_column_indexes == -1:
        ...
        return [i for i in range(num_columns)]
...
def fix_local_corr(remaind_corr, remainds_indexes, size):
    ...
    corr.fill(np.nan)
    if remaind_corr.shape == (0,0):return corr
...
def fix_vif(remains_vif, remainds_indexes, size):
    vif = np.zeros(size)
    vif.fill(np.nan)
    if len(remains_vif) == 0:return vif
...
def fix_corr(remaind_corr, m1, m2, remainds_indexes1, remainds_indexes2):
    corr = np.zeros((m1, m2))
    corr.fill(np.nan)
    if remaind_corr.shape[1] == 0: return corr
