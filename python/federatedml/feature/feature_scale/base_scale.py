...
class BaseScale(object):
    ...
    def _get_scale_column_idx(self, data):
        ...
        # add by tjx 20220804
        elif len(self.param_scale_names)==0:
            scale_column_idx = [i for i in range(data_shape)]
        # add by tjx 20220804
        elif len(self.param_scale_names)>0:
            for i in range(data_shape):
                if data.schema["header"][i] in self.param_scale_names:
                    scale_column_idx.append(i)
    ...
    def __get_min_max_value_by_normal(self, data):
        ...
        if self.feat_upper is not None:
            # add by tjx 20220810
            #if len(scale_column_idx_set) != len(self.feat_upper):
            #    self.feat_upper = self.feat_upper[0]
            if isinstance(self.feat_upper, list):
                """
                remove check as single feature do min max scale by tjx 20220711
                """
                # self.__check_equal(data_shape, len(self.feat_upper))
                self.feat_upper = [u for u in self.feat_upper if u is not None]
                assert len(scale_column_idx_set) == len(self.feat_upper), f"Scale columns don't match upper bounds of feature."
                temp_index = 0
                for i in range(data_shape):
                    if i in scale_column_idx_set:
                        """
                        modify by tjx 20220711 origin code have bug
                        """
                        ...
                        if column_max_value[i] > self.feat_upper[temp_index]:
                            column_max_value[i] = self.feat_upper[temp_index]
                        if column_min_value[i] > self.feat_upper[temp_index]:
                            column_min_value[i] = self.feat_upper[temp_index]
        ...
        if self.feat_lower is not None:
            # add by tjx 20220810
            #if len(scale_column_idx_set) != len(self.feat_lower):
            #    self.feat_lower = self.feat_lower[0]
            if isinstance(self.feat_lower, list):
                """
                remove check as single feature do min max scale by tjx 20220711
                """
                # self.__check_equal(data_shape, len(self.feat_lower))
                self.feat_lower = [u for u in self.feat_lower if u is not None]
                assert len(scale_column_idx_set) == len(self.feat_lower), f"Scale columns don't match lower bounds of feature."
                temp_index = 0
                for i in range(data_shape):
                    if i in scale_column_idx_set:
                        """
                        ...
                        if column_min_value[i] < self.feat_lower[temp_index]:
                            column_min_value[i] = self.feat_lower[temp_index]
                        if column_max_value[i] < self.feat_lower[temp_index]:
                            column_max_value[i] = self.feat_lower[temp_index]
                        temp_index = temp_index + 1
            else:
                for i in range(data_shape):
                    if i in scale_column_idx_set:
