...
class QuantileSummaries(object):
    ...
    def is_nan(self, x):
        ...
        if (x in self.abnormal_list) or (isinstance(x, float) and np.isnan(x)) or (x == NoneType()):
            return True
        return False
...
class SparseQuantileSummaries(QuantileSummaries):
    ...
    def insert(self, x):
        ...
        try:
            ...
            flag = np.isnan(float(x))
        if x in self.abnormal_list or flag:
            self.missing_count += 1
            return
        try:
            flag = x < consts.FLOAT_ZERO
        except Exception as ex:
            flag = float(x)< consts.FLOAT_ZERO

        flag2 = None
        if not flag:
            try:
                flag2 = x >= consts.FLOAT_ZERO
            except Exception as ex:
                flag2 = float(x) >= consts.FLOAT_ZERO
