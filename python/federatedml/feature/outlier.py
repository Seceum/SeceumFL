...
class Outlier(object):
    ...
    def __init__(self, missing_value_list=None,outlier_by_std=1.96,outlier_by_quantile=None):
        """
        Parameters
        ----------
        ...
        else: self.missing_value_list = None
        if outlier_by_quantile is None and outlier_by_std is None:
            self.missing_value_list = missing_value_list
            self.abnormal_value_list = copy.deepcopy(self.missing_value_list)
    ...
    def replace_missing_value_with_cols_transform_value_format(data, transform_list, missing_value_list,
        ...
        if isinstance(_data, Instance):
            for i, v in enumerate(_data.features): # modify for bug 396
                if (v in missing_value_list or v == NoneType()) and i not in skip_cols:
                    _data.features[i] = output_format(transform_list[i])
                    replace_cols_index_list.append(i)
                else:
                    _data[i] = output_format(v)
        else:
            for i, v in enumerate(_data): # modify for bug 396
                if (str(v) in missing_value_list or v == NoneType()) and i not in skip_cols:
    ...
    def replace_missing_value_with_cols_transform_value(data, transform_list, missing_value_list, skip_cols,
                                                        outlier_by_std=1.96,outlier_by_quantile=None,header=None):
        _data = copy.deepcopy(data)
        replace_cols_index_list = []
        if isinstance(_data, Instance):
            for i, v in enumerate(_data.features): # modify for bug 396
                ...
                try:
                    ...
                    isnumeric = False
                if any([
                    (outlier_by_std or outlier_by_quantile) and not isnumeric,
                    (outlier_by_std or outlier_by_quantile) and isnumeric and (v > missing_value_list[header[i]][1] or v < missing_value_list[header[i]][0])
                ]):
    ...
    def replace_missing_value_with_replace_value_format(data, replace_value, missing_value_list, output_format):
        ...
        if isinstance(_data, Instance):
            for i, v in enumerate(_data.features): # modify for bug 396
                if v in missing_value_list or v == NoneType():
            ...
            for i, v in enumerate(_data): # modify for bug 396
                if str(v) in missing_value_list or v == NoneType():
    ...
    @staticmethod
    def __get_cols_transform_method(data, replace_method, col_outlier_fill_method):
        header = get_header(data)
        if col_outlier_fill_method:
            replace_method_per_col = {col_name: col_outlier_fill_method.get(col_name, replace_method) for col_name in header}
        else:
    ...
    def __get_cols_transform_value(self, data, replace_method, replace_value=None,outlier_by_std=1.96,outlier_by_quantile=None):
        """
        ...
        """
        # if 1 in deal_strategies:
        #     summary_obj = MultivariateStatisticalSummary(data, -1, abnormal_list=self.abnormal_value_list)
        # else:
        header = get_header(data)
        headr_idx = {h:i for i,h in enumerate(header)}
        col_idx = [headr_idx[c] for c,m in replace_method.items() if m and c in headr_idx] if replace_method else -1
        summary_obj = MultivariateStatisticalSummary(data, col_idx, abnormal_list=None)
        if outlier_by_std or outlier_by_quantile:
            if (outlier_by_std and outlier_by_quantile is None) or (outlier_by_std is None and outlier_by_quantile):
                if outlier_by_std and outlier_by_quantile is None:
                    """
                    标准差方式判断异常值
                    """
                    ...
                    for i, feature in enumerate(header):
                        if replace_method[feature] is None:
                            ...
                            mean = summary_obj.get_mean()[feature]
                            std = summary_obj.get_std_variance()[feature]
                            upper = mean + outlier_by_std * std
                            lower = mean - outlier_by_std * std
                            self.missing_value_list[feature] = [lower,upper]
                    self.abnormal_value_list = copy.deepcopy(self.missing_value_list)
                elif outlier_by_quantile and outlier_by_std is None:
                    """
                    分位数方式判断异常值
                    """
                    self.missing_value_list = {}
                    for i, feature in enumerate(header):
                        if replace_method[feature] is None:
                            ...
                            percent25 = summary_obj.get_quantile_point(0.25)[feature]
                            percent75 = summary_obj.get_quantile_point(0.75)[feature]
                            upper = percent75 + outlier_by_quantile * (percent75 - percent25)
                            lower = percent25 - outlier_by_quantile * (percent75 - percent25)
                            self.missing_value_list[feature] = [lower, upper]
                    self.abnormal_value_list = copy.deepcopy(self.missing_value_list)
            elif outlier_by_std and outlier_by_quantile:
                #用户同时选择标准差和分位数方式选择异常值

                if outlier_by_std and outlier_by_quantile:
                    self.missing_value_list = {}
                    for i, feature in enumerate(header):
                        if replace_method[feature] is None:
                            continue
                            ...
                            mean = summary_obj.get_mean()[feature]
                            std = summary_obj.get_std_variance()[feature]
                            upper0 = mean + outlier_by_std * std
                            lower0 = mean - outlier_by_std * std
                            #异常值方式正常范围
                            percent25 = summary_obj.get_quantile_point(0.25)[feature]
                            percent75 = summary_obj.get_quantile_point(0.75)[feature]
                            upper1 = percent75 + outlier_by_quantile * (percent75 - percent25)
                            lower1 = percent25 - outlier_by_quantile * (percent75 - percent25)
                            low = lower0
                            if lower1 < lower0:
                                low = lower1
                            high = upper0
                            if upper1 > upper0:
                                high = upper1
                            self.missing_value_list[feature] = [low,high]
                            # if upper0 > lower1 and upper0 < upper1:
                            #     if lower0>lower1:
                            #         self.missing_value_list[feature] = [lower1,upper1]
                            #     elif lower0<lower1:
                            #         self.missing_value_list[feature] = [lower0,upper1]
                            # elif upper1> lower0 and upper1<upper0:
                            #     if lower1>lower0:
                            #         self.missing_value_list[feature] = [lower0,upper0]
                            #     elif lower1<lower0:
                            #         self.missing_value_list[feature] = [lower1,upper0]
                    self.abnormal_value_list = copy.deepcopy(self.missing_value_list)

        cols_transform_value = {}
        # add by tjx 20221110 当指定的替换值不同的时候，每一个特征有自己的替换值，需要用count作为下标获取
        count = 0
        ...
        for i, feature in enumerate(header):
            if replace_method[feature] is None:
                ...
                if isinstance(replace_value, list):
                    transform_value = replace_value[count]
                    count = count + 1
                else:
    ...
    def __fit_replace(self, data, replace_method, replace_value=None, output_format=None,
                      col_outlier_fill_method=None,outlier_by_std=1.96,outlier_by_quantile=None):
        replace_method_per_col, skip_cols = self.__get_cols_transform_method(data, replace_method, col_outlier_fill_method)
        ...
        cols_transform_value = self.__get_cols_transform_value(data, replace_method_per_col,
                                                               replace_value=replace_value,outlier_by_std=outlier_by_std,
                                                               outlier_by_quantile=outlier_by_quantile)
        self.skip_cols = skip_cols
        skip_cols = [get_header(data).index(v) for v in skip_cols]
        if output_format is not None:
            f = functools.partial(Outlier.replace_missing_value_with_cols_transform_value_format,
            ...
            f = functools.partial(Outlier.replace_missing_value_with_cols_transform_value,
                                  transform_list=cols_transform_value, missing_value_list=self.abnormal_value_list,
                                  skip_cols=set(skip_cols),outlier_by_std=outlier_by_std,
                                  outlier_by_quantile=outlier_by_quantile,header=schema["header"])
    ...
    def __transform_replace(self, data, transform_value, replace_area, output_format, skip_cols,outlier_by_std,outlier_by_quantile):
        skip_cols = [get_header(data).index(v) for v in skip_cols]
        if replace_area == 'all':
            if output_format is not None:
                f = functools.partial(Outlier.replace_missing_value_with_replace_value_format,
                                      replace_value=transform_value, missing_value_list=self.abnormal_value_list,
                                      output_format=output_format)
            else:
                f = functools.partial(Outlier.replace_missing_value_with_replace_value,
                                      replace_value=transform_value, missing_value_list=self.abnormal_value_list)
            ...
            if output_format is not None:
                ...
                f = functools.partial(Outlier.replace_missing_value_with_cols_transform_value,
                                      transform_list=transform_value, missing_value_list=self.abnormal_value_list,
                                      skip_cols=set(skip_cols),header=data.schema["header"],outlier_by_std=outlier_by_std,outlier_by_quantile=outlier_by_quantile)
        else:
    ...
    def fit(self, data, replace_method=None, replace_value=None, output_format=consts.ORIGIN,
        ...
        if isinstance(replace_method, str):
            ...
            if replace_method not in self.support_replace_method:
                raise ValueError("Unknown replace method:{}".format(replace_method))
        elif replace_method is None and col_outlier_fill_method is None:
            if isinstance(data.first()[1], Instance):
                replace_value = 0
            else:
                replace_value = '0'
        elif replace_method is None and col_outlier_fill_method is not None:
            LOGGER.debug(f"perform computation on selected cols only: {col_outlier_fill_method}")
        else:
            raise ValueError("parameter replace_method should be str or None only")
        if isinstance(col_outlier_fill_method, dict):
            for col_name, method in col_outlier_fill_method.items():
                method = method.lower()
                if method not in self.support_replace_method:
                    raise ValueError("Unknown replace method:{}".format(method))
                col_outlier_fill_method[col_name] = method

        process_data, cols_transform_value = self.__fit_replace(data, replace_method, replace_value, output_format,
                                                                col_outlier_fill_method=col_outlier_fill_method,
                                                                outlier_by_std=outlier_by_std,outlier_by_quantile=outlier_by_quantile)
    ...
    def transform(self, data, transform_value, output_format=consts.ORIGIN, skip_cols=None,outlier_by_std=1.96,outlier_by_quantile=None):
        """
