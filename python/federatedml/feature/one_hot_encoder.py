...
class OneHotEncoder(ModelBase):
    ...
    def fit(self, data_instances):
        ...
        LOGGER.debug("After transform in fit, schema is : {}, header: {}".format(self.schema,
                                                                                 self.inner_param.header))
        self.one_hot_encoder_distribution(data_instances)
        return data_instances

    def one_hot_encoder_distribution(self,data_instances):
        input_data_data = data_instances.mapValues(lambda value: [v if v else 0 for v in value.features])
        data = input_data_data.collect()
        data = pd.DataFrame(data, columns=["id", "label"])
        data1 = list(data.loc[:, "label"].apply(lambda x: list(eval(x))))
        data2 = pd.DataFrame(data1)
        data2.columns = self.inner_param.result_header
        """
        one hot encoder 直方图，后续待优化
        """
        distribution = {}
        for name in data2.columns:
            bin_num = list(data2[name].unique())
            bin_num = [str(b) for b in bin_num]
            is_label = False
            b = data2.groupby(name).agg({name: "count"})[name]
    ...
    def transform(self, data_instances):
        ...
        # one_data = data_instances.first()[0].features
            ...
            for col_idx, col_name in zip(inner_param.transform_indexes, inner_param.transform_names):
                ...
                if not isinstance(feature_value, str):
                    ...
                    if feature_value != feature[col_idx]:
                        raise ValueError(f"Onehot input data support integer or string only {col_name}：{feature_value}")
