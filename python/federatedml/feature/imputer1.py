...
class Imputer1(object):
    ...
    def replace_missing_value_with_cols_transform_value(data, transform_list, missing_value_list, skip_cols):
        ...
        if isinstance(_data, Instance):
            ...
            count = 0
            for i, v in enumerate(_data):
                if (v in missing_value_list or (v and v!='NA' and float(v) in missing_value_list)) and i not in skip_cols:
                    _data[i] = transform_list[count]
                    replace_cols_index_list.append(count)
                    count = count + 1
    ...
    @staticmethod
    def __get_cols_transform_method(data, replace_method, col_replace_method,feature):
        # specify outlier columns add by tjx 202282
        ...
        if col_replace_method:
            ...
            replace_method_per_col = {col_name: replace_method for col_name in header if col_name in feature}
        skip_cols = [v for v in header if v not in replace_method_per_col.keys()]
        return replace_method_per_col, skip_cols

    def __get_cols_transform_value(self, data, replace_method, replace_value=None,outlier_feature=None):
        """
        ...
        outlier_feature specify which column feature include outlier like x1,x2
        ...
        """
        tempcount index poisition
        """
        tempcount = -1
        for i, feature in enumerate(header):
            # if i == 1:
            #     break
            if outlier_feature and feature not in outlier_feature:
                continue
            else:
            ...
            if replace_method[feature] is None:
                ...
                if isinstance(replace_value, list):
                    transform_value = replace_value[tempcount]
                else:
        ...
        # add by tjx 202282
        cols_transform_value = [cols_transform_value[key] for key in header if key in outlier_feature]
    ...
    def __fit_replace(self, data, replace_method, replace_value=None, output_format=None,
                      col_replace_method=None,outlier_feature=None):
        replace_method_per_col, skip_cols = self.__get_cols_transform_method(data, replace_method, col_replace_method,outlier_feature)
        ...
        cols_transform_value = self.__get_cols_transform_value(data, replace_method_per_col,
                                                               replace_value=replace_value,outlier_feature=outlier_feature)
        self.skip_cols = skip_cols
        skip_cols = [get_header(data).index(v) for v in skip_cols]
        if output_format is not None:
            f = functools.partial(Imputer1.replace_missing_value_with_cols_transform_value_format,
            ...
            f = functools.partial(Imputer1.replace_missing_value_with_cols_transform_value,
                                  transform_list=cols_transform_value, missing_value_list=self.abnormal_value_list,
    ...
    def __transform_replace(self, data, transform_value, replace_area, output_format, skip_cols):
        ...
        if replace_area == 'all':
            if output_format is not None:
                f = functools.partial(Imputer1.replace_missing_value_with_replace_value_format,
                                      replace_value=transform_value, missing_value_list=self.abnormal_value_list,
                                      output_format=output_format)
            else:
                f = functools.partial(Imputer1.replace_missing_value_with_replace_value,
                                      replace_value=transform_value, missing_value_list=self.abnormal_value_list)
    ...
    def fit(self, data, replace_method=None, replace_value=None, output_format=consts.ORIGIN,
        ...
        outlier_feature:which column regard as outlier
        Returns
        ----------
        ...
        process_data, cols_transform_value = self.__fit_replace(data, replace_method, replace_value, output_format,
                                                                col_replace_method=col_replace_method,outlier_feature=outlier_feature)
