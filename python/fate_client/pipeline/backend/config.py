...
def get_default_config() -> dict:
    ...
    try:
        with (Path(__file__).parent.parent / "config.yaml").open(encoding="utf-8") as f:
            ...
            if cnf.get("ip") and cnf["ip"] != "127.0.0.1" and cnf["ip"] != "localhost":
                return cnf
    except Exception as e:
        pass

    with open(os.environ.get("FATE_PROJECT_BASE", "/data/projects/fate/")+"/conf/service_conf.yaml", "r") as f:
        svc_cnf = yaml.safe_load(f)
        cnf["ip"] = svc_cnf["fateflow"]["host"]
        cnf["port"] = svc_cnf["fateflow"]["http_port"]
...
class PipelineConfigMeta(type):
    ...
    def __getattr__(cls, name):
        ...
        if name in {"IP", "PORT"}:
            raise ValueError(
                f"{name} not configured. {cls._conf} === {os.environ.get('FATE_PROJECT_BASE')}"
