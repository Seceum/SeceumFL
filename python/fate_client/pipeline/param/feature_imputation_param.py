...
class FeatureImputationParam(BaseParam):
    ...
    def __init__(self, default_value=0, missing_fill_method=None, col_missing_fill_method=None,
        ...
        if not missing_impute:return

        self.missing_impute = []
        if isinstance(missing_impute, str): missing_impute = missing_impute.split(",")
        for v in missing_impute:
            if isinstance(v, str):
                try:
                    if v.find(".")>=0: v = float(v)
                    else: v = int(v)
                except Exception as e:
                    pass
            self.missing_impute.append(v)
