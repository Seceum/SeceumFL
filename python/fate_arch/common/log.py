...
fateflow_log = re.compile(re.escape('fateflow'),re.IGNORECASE)
fate_flow_log = re.compile(re.escape('fate_flow'),re.IGNORECASE)
fate_log = re.compile(re.escape('fate'),re.IGNORECASE)
eggroll_log = re.compile(re.escape('eggroll'),re.IGNORECASE)

def seceum_log(msg):
    msg = fateflow_log.sub('SeceumFlue', msg)
    msg = fate_flow_log.sub('SeceumFlue', msg)
    msg = fate_log.sub('SeceumFL', msg)
    msg = eggroll_log.sub('SeceumGlue', msg)
    return msg
...
class LoggerFactory(object):
    ...
    def get_handler(class_name, level=None, log_dir=None, log_type=None, job_id=None):
        ...
        if LoggerFactory.log_share:
            # handler = Diy_ROpenHandler(log_file,
            #                            when='D',
            #                            interval=1,
            #                            backupCount=14,
            #                            delay=True)
            ...
            # handler = Diy_TimedRotatingFileHandler(log_file,
            #                                        when='D',
            #                                        interval=1,
            #                                        backupCount=14,
            #                                        delay=True)
...
# class Diy_ROpenHandler(ROpenHandler):
...
#                 stream = self.stream
#                 # stream.write(msg.replace("fate", "seceum"))
#                 stream.write(seceum_log(msg))
#                 stream.write('\n')
#                 self.flush()
#             except Exception:
...
#
#
# class Diy_TimedRotatingFileHandler(TimedRotatingFileHandler):
#     def emit(self, record):
#         try:
#             if self.shouldRollover(record):
#                 self.doRollover()
#             if self.stream is None:
#                 self.stream = TimedRotatingFileHandler._open(self)
#             try:
#                 msg = self.format(record)
#                 stream = self.stream
#                 # stream.write(msg.replace("fate", "seceum"))
...
#
# #
# class Diy_StreamHandler(logging.StreamHandler):
#     def emit(self, record):
#         try:
#             msg = self.format(record)
#             stream = self.stream
