...
class EggRollAddress(AddressBase):
    ...
    def connector(self):
        return {"home": self.home}
class HBASEAddress(AddressBase):
    def __init__(self, host,port,name,namespace="default"):
        self.host = host
        self.port = port
        self.name = name
        self.namespace = namespace
        super(HBASEAddress, self).__init__(connector_name=None)

    def __hash__(self):
        return (self.host, self.port,self.name).__hash__()

    def __str__(self):
        return f"HBASEAddress(host={self.host}, port={self.port}), name={self.name})"
...
class MysqlAddress(AddressBase):
    ...
    def __str__(self):
        return f"MysqlAddress(db={self.db}, name={self.name},user={self.user},passwd={self.passwd},host={self.host},port={self.port})"
...
class OracleAddress(AddressABC):

    def __init__(self, user, passwd, host,port,dsn,name):
        self.host = host
        self.user = user
        self.port = port
        self.dsn = dsn
    ...
    def __hash__(self):
        return (self.host, self.port, self.dsn, self.name).__hash__()

    def __str__(self):
        return f"LinkisOracleAddress(db_username={self.user}, name={self.name},db_host={self.host},db_port={self.port},db_password={self.passwd},db_dsn={self.dsn})"

    def __repr__(self):
        return self.__str__()
class PostgresqlAddress(AddressABC):

    def __init__(self, user, password, host,port,database,name):
        self.host = host
        self.user = user
        self.port = port
    ...
    def __str__(self):
        return f"LinkisPostgresqlAddress(database={self.database}, name={self.name}，host={self.host}，port={self.port}，user={self.user}，password={self.password})"
...
class HiveAddress(AddressBase):
    ...
    def __str__(self):
        return f"HiveAddress(database={self.database}, name={self.name}, host={self.host}, port={self.port}, username={self.username}, password={self.password})"
...
class LinkisHiveAddress(AddressBase):
    ...
    def __str__(self):
        return f"HiveAddress(database={self.database}, name={self.name}, host={self.host}, port={self.port}, username={self.username}, name={self.name})"
