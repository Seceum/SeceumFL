...
class Federation(FederationABC):
    ...
    def get(self, name, tag, parties, gc):
        ...
        raw_result = self._get(name, tag, parties, self._rsc, gc)
    ...
    def _remote_tag_not_duplicate(self, name, tag, parties):
    ...
    def _get(self, name, tag, parties, rsc, gc):
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            v = future.result()
            rtn[party] = self._get_value_post_process(v, name, tag, party, gc)
        return [rtn[party] for party in parties]

    def _get_value_post_process(self, v, name, tag, party, gc):
        log_str = f"federation.eggroll.get.{name}.{tag}"
        if v is None:
            raise ValueError(f"[{log_str}]get `None` from {party}")
        if not self._get_tag_not_duplicate(name, tag, party):
    ...
    def _get_tag_not_duplicate(self, name, tag, party):
