...
from fate_arch.storage._types import PostgresqlStoreType
import pandas as pd
...
class StorageTable(StorageTableBase):
    def __init__(
            ...
            namespace: str = None,
            partitions: int = 1,
            store_type: PostgresqlStoreType = PostgresqlStoreType.storetype_tape,
            options=None,
    ):
    ...
    def get_column_sid(self, id_delimiter=","):
        future_column_sql = "SELECT column_name from information_schema.columns WHERE table_schema = 'public' and table_name = '%s'" % self._address.name

        sid_sql = "select pg_attribute.attname as column_name from \
        ...
        and pg_constraint.contype='p'" % self._address.name
    ...
    def _get_id_feature_name(self):
        ...
        if header:
            ...
            raise Exception("postgresql table need data header")
