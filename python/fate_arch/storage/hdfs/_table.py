...
class StorageTable(StorageTableBase):
    ...
    def read_fl_table(self,sql=None,part_of_data=True):
        if not self._exist():
            return False, 'HDFS 找不到 {}文件'.format(self.path),None,None
        index = 0
        data = []
        columns = []
        with io.TextIOWrapper(buffer=self._hdfs_client.open_input_stream(self.path), encoding="utf-8") as reader:
            for line in reader:
                if part_of_data:
                    if index > 5:
                        break
